...
const CACHE_TTL = 300000 // 5 minutes for stops (they change infrequently)
...
serve(async (req) => {
  ...
  try {
    console.log('🚌 ===== TRANSIT STOPS API CALLED =====')
    ...
    if (cached && (now - cached.timestamp) < CACHE_TTL) {
      console.log('✅ Returning cached stops data')
      return new Response(JSON.stringify({
        success: true,
        stops: cached.data,
        cached: true,
    ...
    // Call CUMTD API
    console.log('🚌 Calling CUMTD getstops API...')
    const cumtdUrl = `${CUMTD_API_BASE}/getstops?key=${CUMTD_API_KEY}`
    ...
    console.log('📊 Stops count:', cumtdData.stops?.length || 0)
    
    // Cache the response
    cache.set(cacheKey, { data: cumtdData.stops, timestamp: now })
    
    return new Response(JSON.stringify({
      success: true,
      stops: cumtdData.stops,
      cached: false,
      cacheAge: 0
    }), {
      headers: { "Content-Type": "application/json" }
    ...
    console.error('❌ ===== TRANSIT STOPS API ERROR =====')
